...
| --- | --- | --- |
| ~~Блокер~~ | Первая версия работает с демонстрационными данными или с реальными учётными записями, SMS, Telegram-входом? | ✅ Решено: демо-данные для POS, реальные аккаунты гостей (только вход через Telegram-бота по deep link, без пароля и SMS) уже реализованы. |
...
- **FR-004**: Система ДОЛЖНА позволять регистрацию/вход нового посетителя исключительно через Telegram-бота по deep link (`t.me/<bot>?start=<token>`), который открывает именно приложение Telegram, а не браузерную версию; отдельной формы email/телефон/пароль нет, аккаунт создаётся автоматически при первом входе через Telegram.
...
- Регистрация и вход выполняются исключительно через Telegram-бота по deep link — форм email/телефон/пароль в системе нет; номер телефона запрашивается отдельным шагом после первого входа, так как он нужен для привязки чеков.
