...
- `tokenHash` — SHA-256 сырого токена (та же техника, что уже используется для верификации Telegram-подписи в `lib/telegram-auth.ts`); сырой токен никогда не попадает в БД, только в одноразовую ссылку.
